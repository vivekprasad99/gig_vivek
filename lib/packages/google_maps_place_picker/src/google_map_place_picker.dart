...
import 'package:awign/packages/google_maps_place_picker/src/components/animated_pin.dart';
import 'package:awign/packages/google_maps_place_picker/src/components/floating_card.dart';
...
class GoogleMapPlacePicker extends StatelessWidget {
  ...
  final TextEditingController selectedAddressController =
      TextEditingController();
  final TextEditingController selectedCityController = TextEditingController();
  final TextEditingController selectedPostalCodeController =
      TextEditingController();
  ...
  _searchByCameraLocation(PlaceProvider provider) async {
    ...
    final GeocodingResponse response =
      ...
      Location(
           lat: provider.cameraPosition!.target.latitude, lng: provider.cameraPosition!.target.longitude,),
      language: language,
    );
  ...
  Widget _buildGoogleMap(BuildContext context) {
    return Selector<PlaceProvider, MapType>(
        ...
        builder: (_, data, __) {
          ...
          return Container(
            margin: const EdgeInsets.only(bottom: Dimens.margin_228),
            child: GoogleMap(
      ...
      child: Selector<PlaceProvider, PinState>(
        ...
        builder: (context, state, __) {
          if (pinBuilder == null) {
            return Container(
              margin: const EdgeInsets.only(bottom: Dimens.margin_228),
              child: _defaultPinBuilder(context, state),
            );
          } else {
            return Container(
              margin: const EdgeInsets.only(bottom: Dimens.margin_228),
  ...
  Widget _buildSelectionDetails(BuildContext context, PickResult? result) {
    selectedAddressController.text = result?.formattedAddress ?? ' ';
    selectedCityController.text = result?.city ?? '';
    selectedPostalCodeController.text = result?.postalCode ?? '';
    return Column(
      ...
      children: <Widget>[
        Padding(
          ...
          child: TextFieldLabel(
              label: 'selected_location'.tr,
              color: Get.theme.captionSecondaryColor),
        ),
        buildSelectedAddressTextField(),
                ...
                child: Column(
                  ...
                  children: [
                    TextFieldLabel(
                        label: 'pincode'.tr,
                        color: Get.theme.captionSecondaryColor),
                    buildSelectedPostalCodeTextField(),
                  ],
                ),
              ),
            ],
        ...
        buildConfirmButton(result),
      ],
    );
  }

  Widget buildConfirmButton(PickResult? result) {
    return RaisedRectButton(
      text: 'confirm'.tr,
      radius: 0,
      backgroundColor: Get.theme.buttonSecondaryColor,
      onPressed: () {
  ...
  Widget buildSelectedAddressTextField() {
    return Padding(
      padding:
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          enabledBorder: UnderlineInputBorder(
            borderSide:
                BorderSide(color: Get.theme.textFieldOutlineColorSecondary),
          ),
  ...
  Widget buildSelectedCityTextField() {
    return TextField(
      style: Get.textTheme.bodyText1,
  ...
  Widget buildSelectedPostalCodeTextField() {
    return TextField(
      style: Get.textTheme.bodyText1,
      controller: selectedPostalCodeController,
      keyboardType: TextInputType.streetAddress,
      maxLines: 1,
      textInputAction: TextInputAction.done,
      decoration: InputDecoration(
