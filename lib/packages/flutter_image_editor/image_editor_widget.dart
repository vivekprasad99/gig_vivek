...
class ImageEditorResult {
  ...
  ImageEditorResult(this.imgWidth, this.imgHeight, this.newFile);
}
...
class ImageEditorWidget extends StatefulWidget {
  ...
  const ImageEditorWidget({Key? key,
  ...
  ///You can extends [ImageEditorWidgetDelegate] and custome it by youself.
  static ImageEditorWidgetDelegate uiDelegate = DefaultImageEditorWidgetDelegate();

  @override
  State<StatefulWidget> createState() {
    return ImageEditorWidgetState();
  }
}
...
class ImageEditorWidgetState extends State<ImageEditorWidget>
  ...
  void saveImage() {
    ...
    //     } else {
    //       Navigator.pop(context, ImageEditorResult(value.width, value.height, file));
    //     }
    //   }).catchError((e) {
    //     Navigator.pop(context);
    //   });
  ...
  static ImageEditorWidgetState? of(BuildContext context) {
    return context.findAncestorStateOfType<ImageEditorWidgetState>();
  }
        ...
        builder: (ctx, value, child) {
          ...
          return Container(
            ...
            child: Column(
              children: [
                12.vGap,
                const Icon(Icons.delete_outline, size: 32, color: Colors.white,),
                4.vGap,
...
/// * If you wanna custom this part, see [ImageEditorWidgetDelegate]
mixin LittleWidgetBinding<T extends StatefulWidget> on State<T> {
  ...
  Widget backWidget({VoidCallback? onPressed}) {
    return GestureDetector(
      ...
      child: ImageEditorWidget.uiDelegate.buildBackWidget(),
    );
  }

  ///operation button in control bar
  Widget getOperateTypeRes(OperateType type, {required bool choosen}) {
    return ImageEditorWidget.uiDelegate.buildOperateWidget(type, choosen: choosen);
  }
  ...
  Widget doneButtonWidget({VoidCallback? onPressed}) {
    return GestureDetector(
      onTap: onPressed,
      child: ImageEditorWidget.uiDelegate.buildDoneWidget(),
    );
  }
  ...
  Widget unDoWidget({VoidCallback? onPressed}) {
    return GestureDetector(
      onTap: onPressed,
      child: ImageEditorWidget.uiDelegate.buildUndoWidget(),
    );
  }
        ...
        builder: (ctx, value, child) {
          return Offstage(
            ...
            child: GestureDetector(
              onTap: onTap,
              child: ImageEditorWidget.uiDelegate.resetWidget,
            ),);
        },
      ),
    );
...
///drawing board
mixin SignatureBinding<T extends StatefulWidget> on State<ImageEditorWidget> {
...
extension _BaseImageEditorWidgetState on State {
  ImageEditorWidgetState? get realState {
    if (this is ImageEditorWidgetState) {
      return this as ImageEditorWidgetState;
    }
    return null;
  }
}
