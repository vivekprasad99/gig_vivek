...
class AttendanceImageInputWidget extends StatefulWidget {
  final Question question;
  final Function(Question question) onAnswerUpdate;
  late final FileConfiguration fileConfiguration;

  AttendanceImageInputWidget(this.question, this.onAnswerUpdate, {Key? key})
      : super(key: key) {
    fileConfiguration = question.configuration as FileConfiguration;
  }

  @override
  State<AttendanceImageInputWidget> createState() => _SyncImageInputWidgetState();
}

class _SyncImageInputWidgetState extends State<AttendanceImageInputWidget> {
  Stream? uploadPercentageStream;
  bool isUploading = false;
  final _attendanceImageInputCubit = sl<AttendanceImageInputCubit>();
  ...
  void initState() {
    ...
    if (widget.question.answerUnit?.hasAnswered() ?? false) {
      ...
      _attendanceImageInputCubit.changeImageSyncStatus(ImageSyncStatus(
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<ImageSyncStatus>(
      stream: _attendanceImageInputCubit.imageSyncStatus,
        ...
        case UploadFromOption.cameraAndGallery:
          return Column(
            children: [
              ...
              Text("OR",style: Get.textTheme.bodyText2SemiBold,),
              const SizedBox(width: Dimens.padding_12),
              buildSelectFromGalleryWidget(context)
            ],
          );
  ...
  Widget buildSelectFromCameraWidget(BuildContext context) {
    return MyInkWell(
      ...
      child: SvgPicture.asset('assets/images/capture_image.svg',)
    );
  }
    ...
    return MyInkWell(
      onTap: () async {
        ...
        LoggingData loggingData = LoggingData(
            event: LoggingEvents.addClicked,
    ...
    if (isPaused) {
      return MyInkWell(
        onTap: () {
          ...
          _attendanceImageInputCubit.upload(
  ...
  _onDeleteTapped() {
    ...
    deleteTap.then((value) {
      if (value == ConfirmAction.OK) {
        _attendanceImageInputCubit.deleteImage(
  ...
  Widget buildFileSizeText(ImageDetails imageDetails, int uploadPercentage) {
    if (imageDetails.getFile() != null) {
      Tuple2<String, String> tuple2 = FileUtils.getFileUploadedSizeAndTotalSize(
          ...
          _attendanceImageInputCubit.totalFileSize,
          uploadPercentage);
      return widget.question.currentRoute == MRouter.arrayQuestionWidget
          ? Expanded(
        child: Text(
  ...
  launchImageEditorWidget(ImageDetails imageDetails) async {
    imageDetails.fromRoute = MRouter.attendanceUploadImageWidget;
