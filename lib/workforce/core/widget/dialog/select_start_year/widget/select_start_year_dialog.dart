...
void showSelectStartYearDialog(
    BuildContext context, Function(int?) onYearTap) {
  showDialog<bool>(
    context: context,
    builder: (_) => CustomDialog(
      child: SelectStartYearDialog(onYearTap),
    ),
  );
}

class SelectStartYearDialog extends StatelessWidget {
  Function(int?) onYearTap;
  List<Widget> yearWidgetList = [];
  List<int> yearList = [];
  late int selectedYear;

  SelectStartYearDialog(this.onYearTap, {Key? key}) : super(key: key) {
  ...
  Widget buildTitle(BuildContext context) {
    return Align(
      ...
      child: Padding(
        ...
        child: Text('choose_start_year'.tr,
  ...
  Widget buildYearPicker() {
    return Container(
      ...
      child: CupertinoPicker(
        ...
        squeeze: 2,
        scrollController:
            FixedExtentScrollController(initialItem: yearWidgetList.length - 1),
